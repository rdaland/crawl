...
string monster_info::_core_name() const
{
    ...
    else
    {
        ...
        case MONS_PANZERGEIST:
            if (inv[MSLOT_ARMOUR])
            {
                const item_def& item = *inv[MSLOT_ARMOUR];
                s = item.name(DESC_PLAIN, false, false, true, false,
                              ISFLAG_KNOW_CURSE | ISFLAG_KNOW_PLUSES) + " geist";
            }
            break;
                             ...
                             bool zombified, bool fullname)
{
    ...
    if (m1.type == MONS_DANCING_WEAPON || m1.type == MONS_PANZERGEIST)
        return false;
...
string monster_info::pluralised_name(bool fullname) const
{
    ...
    if (mons_is_unique(type) && type != MONS_MARA)
        ...
        return pluralise_monster(mons_type_name(MONS_DEMONSPAWN, DESC_PLAIN));
             ...
             || type == MONS_PANZERGEIST || type == MONS_MUTANT_BEAST
             || !fullname)
    {
